...
The Worker solves CORS for the two live tools and caches responses.
...
3. In `bank-health-scanner.html` and `ma-partner-finder.html`, change the one
   `const API` line to:
   ```js
   const API = "https://fdic.bankregwire.com/api";
   ```
   Both tools call `${API}/institutions` and `${API}/financials`; the Worker
   maps `/api/*` straight through to `banks.data.fdic.gov/api/*`.
